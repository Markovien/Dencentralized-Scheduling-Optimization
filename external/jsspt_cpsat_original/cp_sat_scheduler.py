...
solve_jsspt-hf_cp.py

CP-SAT solver for JSSPT-HF with:
- transportation (robots + empty travel)
...
if __name__ == "__main__":
    inst_path = os.path.join("bu_instances_eval/scenario_1", "EX7_1_S1.json")
    inst_name=os.path.basename(inst_path)[:-7].replace('_', '')
    instance = load_jsspt_instance(inst_path)

    status, cmax, cpu, kpi = solve_jsspt_hf_cp(
        instance,
        last_ttask=1,
        num_robots=2,
        time_limit=3600.0,
        ...
        idle_recovery_factor=0.5, # Realistic factor
